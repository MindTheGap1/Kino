...
def searchMovie(request):
	...
	if phrase:
		...
		movie_list = Movie.objects.filter(	reduce(operator.or_, (Q(movieName__icontains=word) for word in phrase_sep))
											...
											| reduce(operator.or_, (Q(genres__genreName__icontains=word) for word in phrase_sep))).distinct().order_by(sorting_list[sort_select]).annotate(
		...
		movie_list = Movie.objects.order_by('movieName').order_by(sorting_list[sort_select]).annotate(
                        isUnwatched=Count('order', filter=Q(order__movieStartTime = None)),
                        startedWatching=Count('order', filter=Q(order__movieStartTime__gte = twoDaysAgo )))
